<x-action-section>
    <x-slot name="title">
        <span class="text-tkblue-500 dark:text-tkblue-500">{{ __('Browser Sessions') }}</span>
    </x-slot>

    <x-slot name="description">
        <span class="text-tkblue-500 dark:text-tkblue-500">{{ __('Manage and log out your active sessions on other browsers and devices.') }}</span>
    </x-slot>
    ...
    <x-slot name="content">
        ...
        <div class="flex items-center mt-5">
            <x-jet-button class="bg-tkteal-500 text-tkblue-500 hover:bg-tkblue-500 hover:text-white" wire:click="confirmLogout" wire:loading.attr="disabled">
        ...
        <x-jet-dialog-modal wire:model="confirmingLogout">
            ...
            <x-slot name="footer">
                <x-jet-button variant="info" wire:click="$toggle('confirmingLogout')" wire:loading.attr="disabled">
                    {{ __('Cancel') }}
                </x-jet-button>

                <x-jet-button class="ml-2 bg-tkteal-500 text-tkblue-500 hover:bg-tkblue-500 hover:text-white" wire:click="logoutOtherBrowserSessions" wire:loading.attr="disabled">
                    {{ __('Log Out Other Browser Sessions') }}
                </x-jet-button>
            </x-slot>
        </x-jet-dialog-modal>
