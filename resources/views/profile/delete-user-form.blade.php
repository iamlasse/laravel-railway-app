<x-action-section>
    <x-slot name="title">
        <span class="text-tkblue-500 dark:text-tkblue-500">{{ __('Delete Account') }}</span>
    </x-slot>

    <x-slot name="description">
        <span class="text-tkblue-500 dark:text-tkblue-500">{{ __('Permanently delete your account.') }}</span>
    </x-slot>
    ...
    <x-slot name="content">
        ...
        <div class="mt-5">
            <x-jet-button variant="danger" wire:click="confirmUserDeletion" wire:loading.attr="disabled">
                {{ __('Delete Account') }}
            </x-jet-button>
        </div>
        ...
        <x-jet-dialog-modal wire:model="confirmingUserDeletion">
            ...
            <x-slot name="footer">
                <x-jet-button variant="info" wire:click="$toggle('confirmingUserDeletion')" wire:loading.attr="disabled">
                    {{ __('Cancel') }}
                </x-jet-button>

                <x-jet-button variant="danger" class="ml-2" wire:click="deleteUser" wire:loading.attr="disabled">
                    {{ __('Delete Account') }}
                </x-jet-button>
            </x-slot>
        </x-jet-dialog-modal>
