		...
		>
			    	...
			    	<div class="flex-1 p-4 text-center">
				    	...
				    	<div>
					    	<div class="relative h-3 max-w-lg mx-auto overflow-hidden rounded-full">
						        ...
						        <div class="absolute h-full bg-telekom-darkgreen" x-bind:style="'width:' + progress + '%'"></div>
						    </div>
				    	</div>
				    </div>
			    @endif
			...
			@if ($mode === 'profile' && $file)
				<div class="absolute inset-0">
					<div class="relative w-full overflow-hidden shadow-inner" style="padding-bottom: 100%">
						...
						@else
							<div class="absolute inset-0 z-10 flex items-center justify-center text-red-500 bg-red-100">
			...
			<div class="flex items-center justify-center flex-1 px-4 py-2" wire:loading.class="hidden" wire:target="{{ $attributes->wire('model')->value }}">
				@if($slot->isEmpty())
					...
					@if ($mode === 'attachment')
						<span class="ml-2 text-gray-600">{{ is_array($file) ? __('Browse files'): __('Browse file') }}</span>
					@endif
				@else
					{{ $slot }}
				@endif
