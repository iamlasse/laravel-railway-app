<div class="bg-tkblue-500">
    <nav aria-label="secondary" x-data="{ open: false }"
        class="sticky top-0 z-10 flex flex-wrap items-center justify-between px-4 py-4 transition-transform duration-500 bg-white rounded-b-xl dark:text-white sm:px-6 dark:bg-tkblue-500"
        :class="{
        ...
        <div class="flex items-center gap-4">
            ...
            @if (Auth::user()->isCompanyUser())
                <div class="flex items-center space-x-2 rep">
                    ...
                    <img class="object-cover w-8 h-8 rounded-md" src="{{ company()->rep->profile_photo_url }}"
                        alt="{{ company()->rep->name }}" />
        ...
        <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden" x-show="false">
            <div class="pt-2 pb-3 space-y-1">
                <x-jet-responsive-nav-link href="{{ route('company.dashboard') }}"
