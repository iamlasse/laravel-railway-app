@props(['variant' => 'primary', 'iconOnly' => false, 'srText' => '', 'href' => false, 'size' => 'base', 'pill' => false, 'squared' => false])
@aware(['disabled' => false])
@php

    $baseClasses = 'focus:outline-none  active:outline-none inline-flex items-center transition-colors font-medium select-none disabled:opacity-50 disabled:bg-opacity-50 disabled:cursor-not-allowed focus:ring focus:ring-offset-2 focus:ring-offset-white dark:focus:ring-offset-dark-eval-2';

    switch ($variant) {
        case 'primary':
            $variantClasses = 'bg-indigo-500 text-white hover:bg-indigo-600 focus:ring-indigo-500';
        break;
        case 'secondary':
            $variantClasses = 'bg-white text-gray-500 hover:bg-gray-100 focus:ring-transparent dark:text-gray-400 dark:bg-dark-eval-1 dark:hover:bg-dark-eval-2 dark:hover:text-gray-200';
        break;
        case 'success':
